...
const GoinglocalPage = ({ data }) => {
  ...
  const goinglocal = data.strapiGoinglocal
  const modules = data.strapiGoinglocal.modules

  const seo = {
    metaTitle: goinglocal.title,
    metaDescription: goinglocal.description,
  }
      ...
      <header className={`main header ${
          ...
          <nav className="language">
            <ul className="menu simple language-selector">
              {goinglocal.localizations.map((goinglocal) => (
                <li>
                  <Link to="/de/goinglocal" target="_self" className="lang-link">DE</Link>
                </li>
              ))}
            </ul>
          </nav>
