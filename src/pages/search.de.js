    ...
    <header className={`main header ${
        ...
        <nav className="language">
          <ul className="menu simple language-selector">
            <li>
              <Link to="/search" target="_self" className="lang-link">EN</Link>
            </li>
          </ul>
        </nav>
          ...
          <article class="module-text cell medium-8">
            <div class="html-content search-revealer">
              <h1 className="text-center">
                <strong>Man findet immer alles dort, wo man es gleich zu Anfang hätte suchen sollen.</strong> Lass uns lustig sein hier?
              </h1>
              <p className="text-center">Einfach lostippen. <b>"Artikel"</b> oder <b>"Zahra"</b>.</p>
            </div>
          </article>
        </div>
      </div>
...
const Hits = ({ hits }) => (
  ...
  <div className="search-hits">
    <div className="grid-x">
      {hits.map(hit => (
        <Link to={`/de/${hit.slug}`} className="cell search-hit" key={hit.objectID}>
