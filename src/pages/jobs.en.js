...
const JobsPage = ({ data }) => {
  ...
  const modules = data.strapiJobs.modules

  const seo = {
    metaTitle: jobs.title,
    metaDescription: jobs.description,
  }
      ...
      <header className={`main header ${
          ...
          <nav className="language">
            <ul className="menu simple language-selector">
              {jobs.localizations.map((jobs) => (
                <li>
                  <Link to="/de/jobs" target="_self" className="lang-link">DE</Link>
                </li>
              ))}
            </ul>
          </nav>
